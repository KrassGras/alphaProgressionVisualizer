...
def generateOneRepMaxCourse(l:list)->dict:
    ...
    listWithoutHeader = removeHeader(l)
...
def generateIntensityGraph(l)->dict:
    filteredData = [set for set in removeHeader(l) if float(set.split(";")[1].replace(",", ".")) != 0]
    course = {"index": [], "intensity":[]}

    for index, set in enumerate(filteredData):
        weight = float(set.split(";")[1].replace(",", "."))
        reps = int(set.split(";")[2])
        oneRM = calculateOneRepMax(weight, reps)
        course["index"].append(f"{index+1}")
        course["intensity"].append((weight/oneRM)*100)

    return course


def removeHeader(l:list)->list:
    listWithoutHeader = [row for row in l if "KG" not in row]
    return listWithoutHeader
...
def extractTrainingdata(csv)->dict:
    ...
    for key in daten.keys():
        daten[key] = [entry for entry in daten[key] if len(entry) < 13 and not entry.isalpha() and len(entry.split(";")) == 3 and "KG" not in entry]
...
if st.button("Exercise statistics") and csv:
    ...
    mostCommonExercise = findMostCommonExercise(st.session_state.training_data)["exerciseName"]
    mostCommonExerciseSets = findMostCommonExercise(st.session_state.training_data)["sets"]
    st.write(f"{mostCommonExercise}\n sets: {mostCommonExerciseSets}")
...
if st.session_state.training_data:
    ...
    if st.button("Show intensity graph"):
        intensityGraph = pd.DataFrame(generateIntensityGraph(st.session_state.training_data[choice]))
        dfIntensityGraph = intensityGraph.set_index("index")
        st.line_chart(dfIntensityGraph)
